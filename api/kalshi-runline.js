export default async function handler(req, res) {
  ...
  try {
    const r = await fetch(
      'https://api.elections.kalshi.com/trade-api/v2/events?series_ticker=KXMLBSPREAD&status=open&with_nested_markets=true&limit=200'
    );
    ...
    const match = events.find(e =>
      e.event_ticker && e.event_ticker.startsWith('KXMLBSPREAD-') && e.event_ticker.endsWith(suffix)
    );
    ...
    // A game may have more than one spread market; return all of them and let the
    // frontend pick whichever line it's currently working with.
    const lines = match.markets.map(market => {
      ...
      const tickerSuffix = market.ticker.split('-').pop(); // e.g. "ATL2"
      const favoredTeam = (tickerSuffix.match(/^[A-Z]+/) || [null])[0];

      // Extract the actual run line from the human-readable subtitle rather than
      // guessing at the ticker's numeric encoding.
      const text = market.yes_sub_title || market.no_sub_title || market.title || '';
